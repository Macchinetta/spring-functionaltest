...
<body>
    ...
    <div id="todoForm">
        <form:form action="${pageContext.request.contextPath}/auth/todo/${id}/list" method="get"
        ...
        <form:form action="${pageContext.request.contextPath}/auth/todo/${id}/02/001" method="get"
            modelAttribute="todoForm">
            <form:hidden path="registrationId" value="${f:h(registrationId)}" />
            <form:hidden path="resourceProtect" value="${f:h(resourceProtect)}" />
            ...
            <c:forEach items="${todos}" var="todo">
                <li>
                    <c:choose>
                        ...
                        <c:otherwise>
                            ${f:h(todo.todoTitle)}
                            <form:form action="${pageContext.request.contextPath}/auth/todo/finish" method="post"
                    ...
                    <form:form action="${pageContext.request.contextPath}/auth/todo/delete" method="post"
                        modelAttribute="todoForm" cssClass="inline">
                        <form:hidden path="todoId" value="${f:h(todo.todoId)}" />
    ...
    <div>
        <c:if test="${not empty accessTokenIssued}">
            <h2>Access Token</h2>
            <p>Access Token Issued : <span id="accessTokenIssued">${accessTokenIssued}</span></p>
            <p>Access Token Expires : <span id="accessTokenExpires">${accessTokenExpires}</span></p>
            ...
            <h2>Refresh Token</h2>
            <p>Refresh Token Issued : <span id="refreshTokenIssued">${refreshTokenIssued}</span></p>
            <p>Refresh Token Expires : <span id="refreshTokenExpires">${refreshTokenExpires}</span></p>
