 ...
 * </p>
 */
public class SimpleValidationTest extends ValidationTestSupport {
        ...
        // Bean Validation
        {
            ...
            // テスト実行(Bean Validation)
            {
                ...
                webDriverOperations.overrideText(id(beanValidationTargets[17]),
                        ld.minusDays(1).toString(super.getLocalDateFormat(currentLocale)));
                webDriverOperations.overrideText(id(beanValidationTargets[18]),
                        ld.minusDays(1).toString(super.getLocalDateFormat(currentLocale)));
                webDriverOperations.overrideText(id(beanValidationTargets[19]),
                        ld.plusDays(1).toString(super.getLocalDateFormat(currentLocale)));
            ...
            // テスト実行(@Positive、@PositiveOrZero等が区別されてチェックされることを確認する)
            {
                ...
                String todayString =
                        new LocalDate().toString(super.getLocalDateFormat(currentLocale));
    ...
    public void testVLDT0101010() {
        ...
        // 実施条件1
        {
            ...
            {
                String errorMessage =
                        webDriverOperations.getText(id("displayErrMessageForm.errors"));
                assertThat(errorMessage,
                        Matchers.allOf(containsString("size must be between 1 and 20"),
                                containsString("size must be between 1 and 50"),
     ...
     * VLDT0101013
     * <ul>
     * <li>エラー時に入力項目以外のスタイルを変更する方法を確認する。(th:classappend="${#fields.hasErrors('fieldName')} ?
     * 'error-label'")</li>
     * </ul>
     */
    @IfProfileValue(name = "test.environment.view", values = {"thymeleaf"})
    ...
    public void testVLDT0101013() {
        ...
        // 実施条件1（エラーになる）
        {
            ...
            // 結果確認
            {
                assertThat(driver.findElement(By.id("labelName")).getAttribute("class"),
                        is("error-label"));
                assertThat(driver.findElement(By.id("labelEmail")).getAttribute("class"),
                        is("error-label"));
                assertThat(driver.findElement(By.id("labelAge")).getAttribute("class"),
                        is("error-label"));
        ...
        // 実施条件2（エラーにならない）
        {
            ...
            // 結果確認
            {
                assertThat(driver.findElement(By.id("labelName")).getAttribute("class"), is(""));
                assertThat(driver.findElement(By.id("labelEmail")).getAttribute("class"), is(""));
                assertThat(driver.findElement(By.id("labelAge")).getAttribute("class"), is(""));
            }
        }
    }
    ...
    private void assertDisplayErrMessageFormLiMessages() {
        WebElement ul = driver.findElement(By.id("displayErrMessageForm.errors"));
        ...
        assertThat(errorMessageList, is(Matchers.containsInAnyOrder("size must be between 1 and 20",
    ...
    public void testVLDT0101014() {
        ...
        // テスト画面表示
        {
            ...
            webDriverOperations
                    .waitForDisplayed(ExpectedConditions.titleContains("Unhandled System Error!"));
        }
