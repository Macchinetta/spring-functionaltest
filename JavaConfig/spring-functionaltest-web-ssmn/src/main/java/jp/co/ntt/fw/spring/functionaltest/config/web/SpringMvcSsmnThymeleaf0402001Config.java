...
@ComponentScan(basePackages = {
        "jp.co.ntt.fw.spring.functionaltest.app.ssmn" }, excludeFilters = {
                ...
                @Filter(type = FilterType.REGEX, pattern = "jp.co.ntt.fw.spring.functionaltest.app.ssmn.SSMN_Thymeleaf_06..*"),
                @Filter(type = FilterType.ASSIGNABLE_TYPE, classes = Cart.class) })
@ComponentScan(basePackages = {
        "jp.co.ntt.fw.spring.functionaltest.app.ssmn" }, excludeFilters = {
                @Filter(type = FilterType.REGEX, pattern = "jp.co.ntt.fw.spring.functionaltest.app.ssmn.SSMN..*") })
@Configuration
@EnableWebMvc
@Import({ SpringMvcCommonConfig.class })
public class SpringMvcSsmnThymeleaf0402001Config implements WebMvcConfigurer {
}
