...
<div id="wrapper">
  ...
  <div>
    <c:if test="${!empty CL_I18N_PRICE_MAP_CODE}">
      <form:select id="cdls0401003_jsp" items="${CL_I18N_PRICE_MAP_CODE}" path="codeListForm.code" />
    </c:if>
  </div>
  <div>CDLS0401004</div>
  <div>
    <c:if test="${!empty CL_I18N_PRICE_MAP_CODE}">
      <form:select id="cdls0401004_jsp" items="${CL_I18N_PRICE_MAP_CODE}" path="codeListForm.code" />
    </c:if>
  </div>
  ...
  <div>
    <c:if test="${!empty CL_MONTH_DES}">
      <form:select id="cdls0402001_jsp" items="${CL_MONTH_DES}" path="codeListForm.code" />
    </c:if>
  </div>
  <div>CDLS0402002</div>
  <div>
    <c:if test="${!empty CL_NUMBER_RANGE_INTERVAL}">
      <form:select id="cdls0402002_jsp" items="${CL_NUMBER_RANGE_INTERVAL}" path="codeListForm.code" />
    </c:if>
  </div>
</div>
  </div>
