...
public class SpringSecuritySpsm05Config {
    ...
    public SecurityFilterChain filterChainJsp0501001(HttpSecurity http,
            @Qualifier("loginForSessionUserInfoManager") AuthenticationManager userLoginManagerForAthn) throws Exception {
        ...
        http.sessionManagement(sessionManagement -> sessionManagement
                .sessionConcurrency(sessionConcurrency -> sessionConcurrency
                        .maxSessionsPreventsLogin(false).maximumSessions(2)
                        .expiredUrl("/")));
        http.sessionManagement(Customizer.withDefaults());
        http.logout(logout -> logout.logoutUrl("/jsp/0501/001/logout")
                .logoutSuccessUrl("/").invalidateHttpSession(true)
                .deleteCookies("JSESSIONID"));
        http.authorizeHttpRequests(authz -> authz.requestMatchers(
                new AntPathRequestMatcher("/jsp/0501/001/login/**")).permitAll()
                .requestMatchers(new AntPathRequestMatcher("/jsp/0501/**"))
    ...
    public SecurityFilterChain filterChainThymeleaf0501001(HttpSecurity http,
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/thymeleaf/0501/001/**"));
        http.authenticationManager(userLoginManagerForAthn);
        ...
        http.sessionManagement(sessionManagement -> sessionManagement
                .sessionConcurrency(sessionConcurrency -> sessionConcurrency
                        ...
                        .expiredUrl("/")));
        http.sessionManagement(Customizer.withDefaults());
        http.logout(logout -> logout.logoutUrl("/thymeleaf/0501/001/logout")
                .logoutSuccessUrl("/").invalidateHttpSession(true)
                .deleteCookies("JSESSIONID"));
        http.authorizeHttpRequests(authz -> authz.requestMatchers(
                new AntPathRequestMatcher("/thymeleaf/0501/001/login/**"))
                .permitAll().requestMatchers(
                        new AntPathRequestMatcher("/thymeleaf/0501/**"))
                .authenticated().requestMatchers(
                        new AntPathRequestMatcher("/**")).permitAll());
    ...
    public SecurityFilterChain filterChainThymeleaf0501002(HttpSecurity http,
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/thymeleaf/0501/002/**"));
        http.authenticationManager(userLoginManagerForAthn);
    ...
    /**
     * Configure {@link AuthenticationManager} bean.
     * @param loginForSessionUserInfoDetailsService Bean defined by #loginForSessionUserInfoDetailsService
