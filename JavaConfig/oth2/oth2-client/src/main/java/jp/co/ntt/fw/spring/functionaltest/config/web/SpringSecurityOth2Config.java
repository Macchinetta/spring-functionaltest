...
import jp.co.ntt.fw.spring.functionaltest.domain.service.interceptor.RestTemplateSendWaitInterceptor;
...
public class SpringSecurityOth2Config {
     ...
     */
    @Value("${oth2.resource.connection.timeout}")
    private Integer connectTimeout;

    /**
     * read.timeout property.
     */
    @Value("${oth2.resource.read.timeout}")
    ...
    public SecurityFilterChain filterChainOAtuh2(HttpSecurity http) throws Exception {
        http.securityMatcher(antMatcher("/jsp/auth/**"));
    ...
    /**
     * Saving authentication requests. Configure {@link AuthorizationRequestRepository} bean.
     * 
    ...
    /**
     * Store and manage authorized clients in memory. Configure
     * {@link OAuth2AuthorizedClientService} bean.
     * 
    ...
    /**
     * Persist OAuth2AuthorizedClient between web requests. Configure
     * {@link OAuth2AuthorizedClientService} bean.
     * 
     * @return Bean of configured {@link OAuth2AuthorizedClientService}
     */
    ...
    public Map<String, RestTemplate> oauthRestTemplateMap() {
        Map<String, RestTemplate> map = new LinkedHashMap<>();
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorReads() {
        return Arrays.asList(tokenInterceptorRead());
    }
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorWrongs() {
        return Arrays.asList(tokenInterceptorWrong());
    }
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorAlls() {
        return Arrays.asList(tokenInterceptorAll());
    }
     ...
     */
    @Bean
    public ClientHttpRequestInterceptor tokenInterceptorChangedTocken() {
        RestTemplateAccessTokenInterceptor bean = new RestTemplateAccessTokenInterceptor();
        bean.setRegistrationId("registration_all");
     ...
     */
    @Bean
    public List<ClientHttpRequestInterceptor> tokenInterceptorChangedTockens() {
        return Arrays.asList(tokenInterceptorChangedTocken());
    }
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorPartials() {
        return Arrays.asList(tokenInterceptorPartial());
    }
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorTokenWrongs() {
        return Arrays.asList(tokenInterceptorTokenWrong());
    }
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorAuthWrongs() {
        return Arrays.asList(tokenInterceptorAuthWrong());
    }
    ...
    public ClientHttpRequestInterceptor tokenInterceptorWait() {
        RestTemplateSendWaitInterceptor bean = new RestTemplateSendWaitInterceptor();
    ...
    public List<ClientHttpRequestInterceptor> tokenInterceptorWaits() {
        return Arrays.asList(tokenInterceptorWait());
    }
     ...
     * @return Bean of configured {@link RestTemplate}
     */
    @Bean("changedTokenRestTemplate")
    public RestTemplate changedTokenRestTemplate() {
        RestTemplate bean = new RestTemplate();
        bean.setRequestFactory(requestFactory());
        bean.setInterceptors(tokenInterceptorChangedTockens());
        bean.setErrorHandler(restTmplateErrorHandler());
        return bean;
    }
}
