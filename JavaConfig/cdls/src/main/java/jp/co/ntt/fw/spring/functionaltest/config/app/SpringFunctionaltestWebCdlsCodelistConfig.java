...
@Configuration
@EnableScheduling
public class SpringFunctionaltestWebCdlsCodelistConfig implements SchedulingConfigurer {
     ...
     */
    @Bean("CL_RELOADABLE_I18N_DB_PRICE")
    public SimpleReloadableI18nCodeList cLReloadableI18nDbPrice() {
        Map<Locale, ReloadableCodeList> rows = new LinkedHashMap<Locale, ReloadableCodeList>();
        rows.put(Locale.ENGLISH, cLDbPriceEn());
        rows.put(Locale.JAPANESE, cLDbPriceJa());
    ...
    public JdbcCodeList cLDbPriceJa() {
        JdbcCodeList jdbcCodeList = abstractJdbcCodeList();
        jdbcCodeList
                .setQuerySql("SELECT code, label FROM t_price WHERE locale = 'ja' ORDER BY code");
        jdbcCodeList.setValueColumn("code");
    ...
    public JdbcCodeList cLDbPriceEn() {
        JdbcCodeList jdbcCodeList = abstractJdbcCodeList();
        jdbcCodeList
                .setQuerySql("SELECT code, label FROM t_price WHERE locale = 'en' ORDER BY code");
        jdbcCodeList.setValueColumn("code");
        jdbcCodeList.setLabelColumn("label");
        return jdbcCodeList;
    }
    ...
    public void configureTasks(ScheduledTaskRegistrar taskRegistrar) {
        ...
        taskRegistrar.addTriggerTask(() -> cLReloadableI18nDbPrice().refresh(),
                new CronTrigger(codelistRefreshTime));
    }
     ...
     */
    @Bean("taskScheduler")
    public Executor taskScheduler2() {
        return Executors.newScheduledThreadPool(10);
    }
    ...
    public DepYearCodeList cLYearCodelist(@Qualifier("clockFactory") ClockFactory clockFactory) {
