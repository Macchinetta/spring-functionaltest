...
public class SpringSecurityRsclConfig {
     ...
     */
    @Bean
    public SecurityFilterChain filterChainRscl(
            HttpSecurity http) throws Exception {

        http.securityMatcher(new AntPathRequestMatcher("/api/v1/rscl/**"));
        ...
        http.authorizeHttpRequests(authz -> authz.requestMatchers(
                new AntPathRequestMatcher("/api/v1/rscl/basic")).hasAuthority(
                        "ROLE_USER").requestMatchers(
    ...
    public UserDetailsService userDetailsService() {
        ...
        manager.createUser(User.builder().username("rscluser").password(
                "password1234").authorities("ROLE_USER").build());
        return manager;
    }
