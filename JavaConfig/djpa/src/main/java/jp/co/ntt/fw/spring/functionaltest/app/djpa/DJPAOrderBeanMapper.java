...
@Mapper
public interface DJPAOrderBeanMapper {

    OrderForm map(JPAOrderForCommonCondition jpaOrderForCommonCondition);

    OrderForm map(JPAOrderForCmnConditionNoBoolean jpaOrderForCmnConditionNoBoolean);

    OrderForm map(JPAOrder jpaOrder);

    OrderItemForm map(JPAOrderItem jpaOrderItem);

    JPAItem map(ItemForm itemForm);

    ItemForm map(JPAItem jpaItem);
}
