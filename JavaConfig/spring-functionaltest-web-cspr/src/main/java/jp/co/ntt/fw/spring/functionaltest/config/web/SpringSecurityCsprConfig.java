...
public class SpringSecurityCsprConfig {
    ...
    @Bean
    @Order(1)
    public SecurityFilterChain filterChainJspNotUseDeniedHandler(
            HttpSecurity http,
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/jsp/notUseDeniedHandler/**"));
        http.authenticationManager(committerAuthenticationManager);
        http.addFilterAfter(userIdMDCPutFilter,
                AnonymousAuthenticationFilter.class);
        http.formLogin(login -> login.loginProcessingUrl("/jsp/authenticate")
                .defaultSuccessUrl("/")
        ...
        http.authorizeHttpRequests(authz -> authz
                .requestMatchers(new AntPathRequestMatcher("/jsp/0301/001")).hasRole("USER")
                .requestMatchers(new AntPathRequestMatcher("/jsp/0301/003")).hasRole("USER")
    ...
    @Bean
    @Order(2)
    public SecurityFilterChain filterChainJsp(
            HttpSecurity http,
            AuthenticationManager committerAuthenticationManager,
            UserIdMDCPutFilter userIdMDCPutFilter,
            CompositeSessionAuthenticationStrategy sessionAuthenticationStrategy,
            @Qualifier("accessDeniedHandler") AccessDeniedHandler accessDeniedHandler) throws Exception {
    ...
    @Bean
    @Order(3)
    public SecurityFilterChain filterChainThymeleafNotUseDeniedHandler(
            HttpSecurity http,
            AuthenticationManager committerAuthenticationManager,
            UserIdMDCPutFilter userIdMDCPutFilter,
            CompositeSessionAuthenticationStrategy sessionAuthenticationStrategy) throws Exception {
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/thymeleaf/notUseDeniedHandler/**"));
        http.authenticationManager(committerAuthenticationManager);
        http.addFilterAfter(userIdMDCPutFilter,
                AnonymousAuthenticationFilter.class);
        http.formLogin(login -> login.loginProcessingUrl("/thymeleaf/authenticate")
                .defaultSuccessUrl("/")
                .loginPage("/thymeleaf/cspr/login"));
        http.logout(logout -> logout.logoutUrl("/thymeleaf/logout")
                .deleteCookies("JSESSIONID"));
        http.sessionManagement(session -> session
                .sessionAuthenticationStrategy(sessionAuthenticationStrategy));
        http.authorizeHttpRequests(authz -> authz
                .requestMatchers(new AntPathRequestMatcher("/thymeleaf/0301/001")).hasRole("USER")
                .requestMatchers(new AntPathRequestMatcher("/thymeleaf/0301/003")).hasRole("USER")
                .requestMatchers(new AntPathRequestMatcher("/**")).permitAll());
    ...
    @Bean
    @Order(4)
    public SecurityFilterChain filterChainThymeleaf(
            HttpSecurity http,
            AuthenticationManager committerAuthenticationManager,
            UserIdMDCPutFilter userIdMDCPutFilter,
            CompositeSessionAuthenticationStrategy sessionAuthenticationStrategy,
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/thymeleaf/**"));
        http.authenticationManager(committerAuthenticationManager);
        http.csrf(csrf -> csrf.csrfTokenRequestHandler(
                csrfTokenRequestAttributeHandler()));
        http.exceptionHandling(ex -> ex.accessDeniedHandler(
    ...
    @Bean
    @Order(5)
    public SecurityFilterChain filterChainJspDisabledcspr(
            HttpSecurity http,
            AuthenticationManager committerAuthenticationManager,
            UserIdMDCPutFilter userIdMDCPutFilter,
            CompositeSessionAuthenticationStrategy sessionAuthenticationStrategy) throws Exception {
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/jspDisabledcspr/**"));
        http.authenticationManager(committerAuthenticationManager);
        ...
        http.formLogin(login -> login.loginProcessingUrl("/jspDisabledcspr/authenticate")
                .defaultSuccessUrl("/")
                .loginPage("/jspDisabledcspr/login"));
        http.logout(logout -> logout.logoutUrl("/jspDisabledcspr/logout")
                .deleteCookies("JSESSIONID"));
        http.sessionManagement(session -> session
                .sessionAuthenticationStrategy(sessionAuthenticationStrategy));
        http.authorizeHttpRequests(authz -> authz
    ...
    @Bean
    @Order(6)
    public SecurityFilterChain filterChainThymeleafDisabledcspr(
            HttpSecurity http,
            AuthenticationManager committerAuthenticationManager,
            UserIdMDCPutFilter userIdMDCPutFilter,
            CompositeSessionAuthenticationStrategy sessionAuthenticationStrategy) throws Exception {
        ...
        http.securityMatcher(
                new AntPathRequestMatcher("/thymeleafDisabledcspr/**"));
        http.authenticationManager(committerAuthenticationManager);
        http.csrf(csrf -> csrf.disable());
        http.addFilterAfter(userIdMDCPutFilter,
                AnonymousAuthenticationFilter.class);
        http.formLogin(login -> login.loginProcessingUrl("/thymeleafDisabledcspr/authenticate")
                .defaultSuccessUrl("/")
                .loginPage("/thymeleafDisabledcspr/login"));
     ...
     * @param committerPasswordAuthenticationProvider Bean defined by #committerPasswordAuthenticationProvider
