<div class="row">
    ...
    <table class="table table-striped table-bordered table-condensed">
        ...
        <c:forEach items="${page.content}" var="order" varStatus="rowStatus">
            <tr>
                ...
                <td id="statusCode_${f:h(order.id)}"><a id="idStatus${f:h(order.id)}" href="${pageContext.request.contextPath}/dam3mb3/ordermb3/detailCondSts/${f:h(order.id)}/${f:h(order.orderStatus.code)}">${f:h(order.orderStatus.code)}</a></td>
                <td id="statusName_${f:h(order.id)}">${f:h(order.orderStatus.name)}</td>

                <td id="quantity_${f:h(order.id)}">
                    <c:forEach items="${order.orderItems}" var="item">
                        <fmt:formatNumber value="${item.quantity}" pattern="###,###" /><br/>
                    </c:forEach>
                </td>
                ...
                <td id="itemPrice_${f:h(order.id)}">
                    <c:forEach items="${order.orderItems}" var="item">
                         <fmt:formatNumber value="${item.item.price}" pattern="###,###" /><br/>
                    </c:forEach>
                </td>
    ...
    <div class="paginationPart" align="center">
        <t:pagination page="${page}" disabledHref="javascript:void(0);"
            ...
            criteriaQuery="${f:query(orderMB3Form)}" />
    </div>
    <div>
        <table class ="table-striped table-bordered table-condensed">
            <tr>
                ...
                <c:forEach items="${searchItemCode}" var="searchItemCode" varStatus="rowStatus">
                    <tr>
                        <td>
                            <form:checkbox id="checkbox_${searchItemCode.code}" path="searchItemCode" value="${searchItemCode.code}" label="${searchItemCode.code}"/>
                        </td>
                    </tr>
                </c:forEach>
                <tr>
                    <td>
                        <form:button name="itemCodeSearch" class="btn btn-default">Show the orders that contains the item code</form:button>
                    </td>
                </tr>
        </table>
    </div>
