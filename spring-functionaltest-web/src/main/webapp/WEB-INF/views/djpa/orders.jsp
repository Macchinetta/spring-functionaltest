	...
	<div>
		<table class="table table-striped table-bordered table-condensed">
				<thead>
            		...
            		action="${pageContext.request.contextPath}/djpa/order" method="get">
					<tr>
        ...
        <c:forEach items="${orderList}" var="order" varStatus="rowStatus">
            <tr>
            	...
            	<c:choose>
            		...
            		<td id="orderId_${rowStatus.count}"><a id="id${f:h(rowStatus.count)}" href="${pageContext.request.contextPath}/djpa/order/cmnCondFetch/${f:h(order.orderId)}/detail">${f:h(order.orderId)}</a></td>
            		</c:when>
            		<c:otherwise>
            		<td id="orderId_${rowStatus.count}"><a id="id${f:h(rowStatus.count)}" href="${pageContext.request.contextPath}/djpa/order/${f:h(order.orderId)}/detail">${f:h(order.orderId)}</a></td>
            		</c:otherwise>
                ...
                <c:choose>
                		<c:when test="${order.logicalDelete}"><c:set var="canDelete" value="Yes"></c:set></c:when>
                		<c:otherwise><c:set var="canDelete" value="No"></c:set></c:otherwise>
               		</c:choose>
