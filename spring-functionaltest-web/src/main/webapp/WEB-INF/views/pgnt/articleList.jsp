...
<div class="text-center">
  <form:form
    action="${pageContext.request.contextPath}/pgnt/${f:hjs(path)}"
    method="get" modelAttribute="articleSearchCriteria">
    <fieldset>
...
<c:if test="${page != null}">
  ...
  <table class="table table-striped table-bordered table-condensed pgnttable">
    ...
    <c:forEach var="article" items="${page.content}" varStatus="rowStatus">
      <tr>
        <td>
            ${f:h(article.articleId)}
        </td>
        <td>
            ${f:h(article.category)}
        </td>
        <td>
            <a href="">${f:h(article.title)}</a>
        </td>
        <td>
