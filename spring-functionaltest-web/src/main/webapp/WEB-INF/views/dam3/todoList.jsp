<div class="row">
	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<c:when test="${mapRes == true}">
    		...
    		<tr>
				...
				<td class="releaseDate" id="createdDate"> <fmt:formatDate
                        pattern="yyyy/MM/dd"
				...
				<td class="releaseDate" id="completeAt"> <joda:format
                        pattern="yyyy/MM/dd"
		...
		<c:when test="${isPaginated == true}">
            ...
            <tr>
               ...
               <td class="title" id="title">${f:h(todo.todoTitle)}</td>
				<td class="releaseDate" id="createdDate">  <fmt:formatDate
                        value="${todo.createdAt}" pattern="yyyy/MM/dd" /></td>
				<c:choose>
				...
				<td class="categoryName" id="categoryName"><a
                    id="todoLink_${rowStatus.count}"
                    href="${pageContext.request.contextPath}/dam3/todo/${todo.todoId}/update">
                        Refer Details Page</a></td>
                <td class="releaseDate" id="completeAt"> <joda:format
                        pattern="yyyy/MM/dd"
			...
			<tr>
				...
				<td class="bookId"><a id="todoLink_${todo.todoId}"
					href="${pageContext.request.contextPath}/dam3/todo/${todo.todoId}/update">
						${f:h(todo.todoId)}</a></td>
				<td class="title" id="title">${f:h(todo.todoTitle)}</td>
				<td class="releaseDate" id="createdDate"> <%-- <joda:format
                        pattern="yyyy/MM/dd"
	...
	<!-- This choose tag is used to identify whether the pagination is done by standard MyBatis method or the SQL refinement method. -->
		<c:choose>
			<c:when test="${isSQLRfinePageSrch == true}"><c:set var="paginationContextName" value="sqlRefPageSearch"></c:set></c:when>
			<c:otherwise><c:set var="paginationContextName" value="stdPageSearch"></c:set></c:otherwise>
		</c:choose>
	 <fmt:formatDate value="${todoForm.createdAt}"
                            pattern="yyyy/MM/dd" var="createdAt" />
	...
	<div>
		<form:form modelAttribute="todoForm"
			...
			<table class="table-striped table-bordered table-condensed">
				<tr>
					...
					<td>Enter Todo creation date : <fmt:formatDate value="${todoForm.createdAt}"
                            pattern="yyyy/MM/dd" var="createdAt" /><form:input path="createdAt" class="form-control" value="${createdAt }" /></td>
					</tr>
				</table>
				</fieldset>
				<fieldset>
			<legend>Action Buttons</legend>
				...
				<tr>
				<td>Get Todo Count for Given Status-->::&nbsp;Incomplete&nbsp;<form:radiobutton  id="incomplete" path="finished"  value="0" />&nbsp;&nbsp;Finished&nbsp;<form:radiobutton id="complete" path="finished"  value="1" /></td>
				<td colspan="1">
					<form:button name="countByStatus" class="btn btn-default">Get Count</form:button>&nbsp;<form:button name="ifSrch" class="btn btn-default">IF element demo search</form:button>
				</td>
