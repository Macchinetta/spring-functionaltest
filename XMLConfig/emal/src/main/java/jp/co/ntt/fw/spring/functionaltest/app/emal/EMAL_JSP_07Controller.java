...
@Controller
@RequestMapping("jsp")
public class EMAL_JSP_07Controller {
    ...
    public String handJndiContainerMail(Model model, EmailSendingForm form,
        ...
        try (Store store = jndiMailSendingService.popBeforeSmtp()) {
            jndiMailSendingService.sendJndiContainerMailSender(form.getTo().get(0), form.getText(),
                    store);
    ...
    public String handJndiOriginMail(Model model, EmailSendingForm form, RedirectAttributes attrs) {

        try (Store store = jndiMailSendingService.popBeforeSmtp()) {
            jndiMailSendingService.sendJndiOriginMailSender(form.getTo().get(0), form.getText(),
                    store);
        } catch (MessagingException e) {
            // ignore
        }
