...
public class TodoRestWrongController {
    ...
    // Controllerは準備しておくが、当Contorollerまでリクエストが飛んでくることはない想定
    // （JWTエンドポイントを間違って記載しているため）
    ...
    private List<TodoResource> getTodos() {
        ...
        for (Todo todo : todos) {
            todoResources.add(this.beanMapper.map(todo, TodoResource.class));
        }
