...
| Beanマッピング(MapStruct)| Bean Mapping | BNMP |
...
-   @RequestMapping、@GetMapping、@PostMappingアノテーションに指定する値となる。
...
-   @GetMapping、@PostMappingのvalue属性には、分割したサーブレットに割り振ったサーブレットパスからの相対パス部分のみでよい。
