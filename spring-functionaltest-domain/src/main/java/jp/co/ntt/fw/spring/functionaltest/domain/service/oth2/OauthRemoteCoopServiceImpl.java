...
@Transactional
@Service
public class OauthRemoteCoopServiceImpl implements OauthRemoteCoopService {

    private static final Logger logger = LoggerFactory.getLogger(
            OauthRemoteCoopServiceImpl.class);

    @Value("${oth2.remoteApplicationContextUrl}${oth2.restServletPath}/oth2/resources")
    String url;

    @Inject
    @Named("authorizationCodeRemoteRestTemplate")
    RestOperations restOperations;

    @Override
    public OauthResource getResource(String testId) {
        final String resourceServerUrl = url + "/extend/{id}";
    ...
    @Override
    public OauthResource getResourceDefault(String testId) {
        final String resourceServerUrl = url + "/string/{id}";
        logger.debug("RestOperation url={}, parameter={}", resourceServerUrl,
                testId);
    ...
    public OauthResource getResourceUnauthorized(String testId) {
        final String resourceServerUrl = url + "/unauth/{id}";
        logger.debug("RestOperation url={}, parameter={}", resourceServerUrl,
                testId);
